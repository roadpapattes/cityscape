...
class GameStep {
  ...
  factory GameStep.fromJson(Map<String, dynamic> j) {
    ...
    // Hints: liste prioritaire, sinon fallback sur string unique
    List<String> parsedHints = const [];
    final rawHints = j['hints'];
    if (rawHints is List) {
      parsedHints = rawHints.map((e) => '$e').where((s) => s.trim().isNotEmpty).toList();
    } else {
...
class ApiService {
  ...
  if (token != null) {
    // Authenticated: no cache + include Authorization so the API can filter private items
    ...
    // Anonymous: keep the cache (separate key)
  ...
  if (token != null) {
    // Authenticated: no cache + Authorization
    final r = await _get(uri, headers: {
      'Authorization': 'Token $token',
      'Accept': 'application/json',
    });
