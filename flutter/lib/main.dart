...
// const String baseUrl = "https://roadpapattes.synology.me/api";
const baseUrl = String.fromEnvironment('BASE_URL', defaultValue: 'https://roadpapattes.synology.me/api');
...
class _HomeListPageState extends State<HomeListPage> {
  ...
  Future<void> _loadNearby() async {
    ...
    try {
      final lat = double.parse(_ _latCtrl.text.trim());
...
class _MapPageState extends State<MapPage> {
  ...
  Future<void> _aroundMe() async {
    try {
      ...
      final items = await _api.fetchNearby(lat: pos.latitude, lon: pos.longitude, radiusKm: 5);
      final markers = items.map((e) => Marker(
        markerId: MarkerId("eg_${e.id}"),
        position: LatLng(e.latitude, e.longitude),
        infoWindow: InfoWindow(title: e.title, snippet: "${e.city} • ${e.rating.toStringAsFixed(1)}★"),
...
Future<Position> _getPosition() async {
  ...
  if (permission == LocationPermission.deniedForever) {
    throw Exception("Permission refusée définitivement. Va dans les paramètres.");
  }
